...
fn test_image_ref_parsing() {
    ...
    let test_cases = vec![
        ...
        ("my-registry.com:5000/my-image:tag", ("my-registry.com:5000", "my-image", "tag")),
    ];

    for (input, (expected_registry, expected_repo, expected_tag)) in test_cases {
        let parsed = parse_image_ref(input);
        assert_eq!(parsed.registry, expected_registry, "Registry mismatch for: {}", input);
        assert_eq!(parsed.repository, expected_repo, "Repository mismatch for: {}", input);
