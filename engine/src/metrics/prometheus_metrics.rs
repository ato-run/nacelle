use anyhow::Result;
use prometheus::{
    Encoder, Gauge, GaugeVec, IntGauge, Opts, Registry, TextEncoder,
};
use std::sync::Arc;
use tracing::debug;
...
impl MetricsCollector {
    ...
    pub fn new() -> Result<Self> {
        ...
        let capsule_status = GaugeVec::new(
            Opts::new("capsule_status", "Status of capsules (1=running, 0=stopped)")
                .namespace("capsuled")
                .subsystem("engine"),
            &["capsule_id", "status"],
        )?;
    ...
    pub fn remove_capsule_metrics(&self, capsule_id: &str) {
        ...
        let _ = self.container_cpu_usage
            .remove_label_values(&[capsule_id]);
        
        // Remove capsule status metrics
        for status in &["pending", "running", "stopped", "failed"] {
            let _ = self.capsule_status
