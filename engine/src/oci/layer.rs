...
impl LayerExtractor {
    ...
    fn handle_whiteout(&self, whiteout_path: &Path, rootfs: &Path) -> LayerResult<()> {
        let filename = whiteout_path.file_name()
            .and_then(|f| f.to_str())
    ...
    pub fn merge_layers(&self, layer_paths: &[PathBuf], rootfs: &Path) -> LayerResult<()> {
        ...
        info!("Merging {} layers into {}", layer_paths.len(), rootfs.display());
        
        for (i, layer_path) in layer_paths.iter().enumerate() {
            info!("Extracting layer {}/{}: {}", i + 1, layer_paths.len(), layer_path.display());
    ...
    pub fn verify_layer_digest(&self, layer_path: &Path, expected_digest: &str) -> LayerResult<bool> {
        let file = File::open(layer_path)?;
...
mod tests {
    ...
    fn test_extract_simple_layer() {
        ...
        let tar_path = create_test_tarball(temp.path(), &[
    ...
    fn test_layer_merge() {
        ...
        // Layer 1: base files
        let layer1 = create_test_tarball(temp.path(), &[
        ...
        header.set_mode(0o644);
        header.set_cksum();
        builder.append_data(&mut header, "shared.txt", "Modified".as_bytes()).unwrap();
        ...
        header.set_mode(0o644);
        header.set_cksum();
        builder.append_data(&mut header, "new.txt", "New".as_bytes()).unwrap();
        ...
        extractor.merge_layers(&[layer1_renamed, layer2_path], &rootfs).unwrap();
