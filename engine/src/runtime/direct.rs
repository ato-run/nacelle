    ...
    impl DirectRuntime {
        ...
        async fn prepare_bundle(
            ...
            if !bundle_path.exists() {
                info!("Preparing bundle for {} (image: {}) at {:?}", container_id, image, bundle_path);
                ...
                let rootfs_path = if !image.is_empty() && image != "dummy" && image != "dummy-image" {
                    ...
                    match self.image_manager.pull_image(image).await {
                        Ok(pulled) => {
                            info!("  Image pulled successfully: {} layers, {} bytes", 
                        ...
                        Err(e) => {
                            warn!("  Failed to pull image: {:?}, falling back to bind-mount mode", e);
        ...
        async fn prepare_fallback_rootfs(&self, bundle_path: &PathBuf) -> Result<PathBuf, RuntimeError> {
            let rootfs_path = bundle_path.join("rootfs");
            std::fs::create_dir_all(&rootfs_path).map_err(RuntimeError::Io)?;
            
            // Create essential directories
            for dir in ["bin", "lib", "usr", "proc", "dev", "sys", "tmp", "etc", "root"] {
        ...
        fn write_config_json(
            ...
            if let Some(host_mounts) = mounts {
                for m in host_mounts {
                    ...
                    if all_mounts.iter().any(|x| x.destination().to_str() == Some(dest.as_str())) {
                        continue;
                    }
        ...
        async fn spawn(&self, container_id: &str, bundle_path: PathBuf) -> Result<(), RuntimeError> {
            info!("Spawning container directly: {} from {:?}", container_id, bundle_path);
            ...
            tokio::task::spawn_blocking(move || {
                let mut container = ContainerBuilder::new(container_id.clone(), SyscallType::Linux)
                    ...
                    .map_err(|e| RuntimeError::Libcontainer(format!("Failed to set root path: {}", e)))?
                    .as_init(&bundle_path)
                    .with_systemd(false)
                    .build()
                    .map_err(|e| RuntimeError::Libcontainer(format!("Failed to build container: {}", e)))?;
                
                container.start()
                    .map_err(|e| RuntimeError::Libcontainer(format!("Failed to start container: {}", e)))?;
                
                let pid = container.pid().ok_or_else(|| RuntimeError::Libcontainer("Failed to get PID after start".to_string()))?;
                ...
                match nix::sys::wait::waitpid(Pid::from_raw(pid.as_raw()), None) {
                     Ok(status) => info!("Container {} exited with status {:?}", container_id, status),
        ...
        pub async fn delete(&self, container_id: &str) -> Result<(), RuntimeError> {
             ...
             tokio::task::spawn_blocking(move || {
                ...
                let mut container = Container::load(root_dir.join(&container_id))
                    .map_err(|e| RuntimeError::Libcontainer(format!("Failed to load container for deletion: {}", e)))?;
...
mod other_impl {
    ...
    impl DirectRuntime {
        ...
        pub async fn spawn(&self, _container_id: &str, _bundle_path: PathBuf) -> Result<(), RuntimeError> {
