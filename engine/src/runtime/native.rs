...
impl Runtime for NativeRuntime {
    async fn launch(&self, request: LaunchRequest<'_>) -> Result<LaunchResult, RuntimeError> {
        ...
        let execution = if let Some(json) = request.manifest_json {
            ...
            None
        }.ok_or_else(|| RuntimeError::InvalidConfig("Missing native execution config".to_string()))?;
        ...
        let parts = shell_words::split(&raw_entrypoint).unwrap_or_else(|_| vec![raw_entrypoint.clone()]);
        ...
        let binary_path = match name_part {
            ...
            _ if std::path::Path::new(name_part).is_absolute() && std::path::Path::new(name_part).exists() => {
        ...
        let final_args: Vec<String> = effective_args.iter().map(|arg| {
