...
impl CapsuleVerifier {
    ...
    pub fn verify(&self, content: &[u8], sig: &CapsuleSignature) -> Result<()> {
        ...
        if sig.algorithm != "ed25519" {
            return Err(anyhow!("Unsupported signature algorithm: {}", sig.algorithm));
        }
        ...
        // 3. Decode public key
        let key_bytes = BASE64.decode(&sig.public_key)
            .map_err(|e| anyhow!("Invalid base64 public key: {}", e))?;
...
mod tests {
    ...
    fn test_verify_untrusted_key() {
        ...
        let result = verifier.verify(content, &signature);
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("not in trusted set"));
    }
    ...
    fn test_multiple_trusted_keys() {
        ...
        let verifier = CapsuleVerifier::new(vec![
            signer1.public_key(),
            signer2.public_key(),
        ])
        .unwrap();
