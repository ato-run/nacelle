...
if [[ "$SCRIPT_DIR" == *".git/hooks"* ]]; then
    ...
    RUNNING_AS_GIT_HOOK=1
else
    # Running directly from scripts/
    PROJECT_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
    RUNNING_AS_GIT_HOOK=0
fi
...
# Git hook のときは、長いチェックでタイムアウトしやすいため
# 「すでに手動で検証を実行したか？」だけ確認して、y なら即 OK / n なら中断します。
if [ "${RUNNING_AS_GIT_HOOK}" -eq 1 ] && [ -t 0 ]; then
    echo -e "${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${YELLOW}確認:${NC} すでに手動で pre-push 検証を実行しましたか？"
    echo -e "${BLUE}y${NC}: 実行済みとして push を続行 / ${BLUE}n${NC}: 手順を表示して中断"
    echo -e "${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    read -r -p "Choice [y/n]: " choice
    case "$choice" in
        y|Y)
            echo ""
            echo -e "${GREEN}✅ OK: 手動実行済みとして pre-push チェックをスキップします${NC}"
            exit 0
            ;;
        n|N)
            echo ""
