...
func TestGpuSimulationE2E(t *testing.T) {
	...
	_, err = testDB.Exec(`
		...
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, "rig-a", testNodeAddr1, "rig-a", "active", 0, now, now, now, vramRTX4090Dual, vramUsageHigh, "12.2")
	require.NoError(t, err)
	...
	_, err = testDB.Exec(`
		INSERT INTO nodes (id, address, headscale_name, status, is_master, last_seen, created_at, updated_at, total_vram_bytes, used_vram_bytes, cuda_driver_version)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, "rig-b", testNodeAddr2, "rig-b", "active", 0, now, now, now, vramRTX4090Single, vramUsageLight, "12.2")
	require.NoError(t, err)
	...
	_, err = testDB.Exec(`
		INSERT INTO nodes (id, address, headscale_name, status, is_master, last_seen, created_at, updated_at, total_vram_bytes, used_vram_bytes, cuda_driver_version)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, "rig-c", testNodeAddr3, "rig-c", "active", 0, now, now, now, vramA100Quad, vramUsageNearly, "12.2")
	require.NoError(t, err)
