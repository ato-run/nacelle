...
"#;
	...
	assert_eq!(docker.mounts[0].source, "/tmp/gumball-test/volumes/hello-docker-storage/data");
	assert_eq!(docker.mounts[0].target, "/data");
	assert!(!docker.mounts[0].readonly);
	assert_eq!(docker.mounts[1].source, "/tmp/gumball-test/volumes/hello-docker-storage/models");
