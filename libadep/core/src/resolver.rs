...
impl Resolver {
    pub fn resolve(draft: &DraftInput, source_path: &Path) -> Result<CapsuleManifestV1> {
        let name = draft.name.clone()
            .or_else(|| source_path.file_name().and_then(|n| n.to_str()).map(|s| s.to_string()))
    ...
    fn detect_type(path: &Path) -> Result<DraftType> {
        ...
        Err(anyhow!("Could not auto-detect project type. Please specify type in draft or capsule.toml."))
    }

    fn resolve_app(path: &Path, draft: &DraftInput) -> Result<(RuntimeType, String, Option<u16>, CapsuleRequirements)> {
        // Node
        if path.join("package.json").exists() {
            // Check scripts.start
            let pkg_json = fs::read_to_string(path.join("package.json"))?;
            if !pkg_json.contains("\"start\"") { // Simple check, ideally parse JSON
                return Err(anyhow!("package.json missing `scripts.start`. Please define a start script."));
            }
            return Ok((RuntimeType::Docker, "npm start".to_string(), Some(3000), CapsuleRequirements::default()));
        }
        ...
        if path.join("requirements.txt").exists() {
             ...
             return Ok((RuntimeType::Docker, entry, Some(8000), CapsuleRequirements::default()));
        }

        Err(anyhow!("Unsupported App type"))
    }

    fn resolve_tool(path: &Path, draft: &DraftInput) -> Result<(RuntimeType, String, Option<u16>, CapsuleRequirements)> {
        // Assuming Python for Tools in v0 as per spec (requirements.txt + mcp)
         ...
         Ok((RuntimeType::Docker, entry, Some(8000), CapsuleRequirements::default()))
    }
...
mod tests {
    ...
    fn test_detect_node_app() -> Result<()> {
        ...
        let draft = DraftInput { name: Some("node-app".into()), ..Default::default() };
    ...
    fn test_detect_python_tool() -> Result<()> {
        ...
        let draft = DraftInput { name: Some("py-tool".into()), ..Default::default() };
        let manifest = Resolver::resolve(&draft, dir.path())?;
    ...
    fn test_detect_python_app_fallback() -> Result<()> {
        ...
        let draft = DraftInput { name: Some("py-app".into()), ..Default::default() };
        let manifest = Resolver::resolve(&draft, dir.path())?;
        assert_eq!(manifest.capsule_type, CapsuleType::App);
    ...
    fn test_detect_static() -> Result<()> {
        ...
        let draft = DraftInput { name: Some("static".into()), ..Default::default() };
        let manifest = Resolver::resolve(&draft, dir.path())?;
        assert_eq!(manifest.capsule_type, CapsuleType::App);
