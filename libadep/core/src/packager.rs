...
impl<'a> Packager<'a> {
    ...
    pub fn plan(
        ...
        let registry = self
            ...
            .registry_host
            .as_deref()
            .unwrap_or("registry.gumball.dev");
        ...
        match manifest.execution.runtime {
            RuntimeType::Docker => {
                ...
                if manifest.execution.entrypoint.contains("python") {
                    ...
                    let parts: Vec<&str> =
                        manifest.execution.entrypoint.split_whitespace().collect();
                    let cmd_json =
                ...
                Err(anyhow!(
                    "Unsupported runtime/entrypoint for auto-generation: {}",
...
mod tests {
    use super::*;
    use crate::capsule_v1::{CapsuleExecution, CapsuleManifestV1, CapsuleType, RuntimeType};
    ...
    fn test_plan_release() {
        ...
        let plan = packager
            .plan(&manifest, Path::new("."), BuildMode::Release)
            .unwrap();

        assert_eq!(plan.primary_tag, "registry.gumball.dev/ekoh/test-app:0.1.0");
        assert_eq!(
            plan.additional_tags,
            vec!["registry.gumball.dev/ekoh/test-app:latest"]
        );
    ...
    fn test_plan_snapshot() {
        ...
        let plan = packager
            .plan(&manifest, Path::new("."), BuildMode::Snapshot)
            .unwrap();
