...
impl DraftInput {
    pub fn merge(&mut self, other: DraftInput) {
        if let Some(name) = other.name { self.name = Some(name); }
        if let Some(version) = other.version { self.version = Some(version); }
        if let Some(display_name) = other.display_name { self.display_name = Some(display_name); }
        if let Some(icon) = other.icon { self.icon = Some(icon); }
        if let Some(description) = other.description { self.description = Some(description); }
        if let Some(tags) = other.tags { self.tags = Some(tags); }
...
impl DraftAdvanced {
    pub fn merge(&mut self, other: DraftAdvanced) {
        if let Some(t) = other.type_ { self.type_ = Some(t); }
        if let Some(s) = other.start { self.start = Some(s); }
        if let Some(p) = other.port { self.port = Some(p); }
        if let Some(hc) = other.health_check { self.health_check = Some(hc); }
        if let Some(bi) = other.base_image { self.base_image = Some(bi); }
...
mod tests {
    ...
    fn test_draft_merge() {
        ...
        assert_eq!(base.advanced.as_ref().unwrap().start, Some("npm start".to_string()));
    }
}
