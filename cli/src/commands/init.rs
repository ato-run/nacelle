...
pub fn execute(args: InitArgs) -> Result<()> {
    ...
    if manifest_path.exists() {
        anyhow::bail!(
            ...
            Use 'nacelle dev --manifest capsule.toml' to run, or delete the file to re-initialize."
        );
    }
    ...
    println!("\nNext steps:");
    println!("   nacelle dev           # Run locally (no bundling)");
    println!("   nacelle pack          # Create deployable archive (.capsule)");
    println!("   nacelle pack --bundle # Create self-extracting bundle");
