...
Capsuled は Personal Cloud OS のコア実装で、分散コンピューティング環境において複数の Engine ノードを管理し、Capsule（コンテナ）のデプロイメントとスケジューリングを行うシステムです。

### 主な特徴

- **分散アーキテクチャ**: 複数のクライアント・エンジンノードによるクラスター構成
